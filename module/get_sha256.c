...
char *get_sha256(const char *path, char *buf)
{
	...
	if (!(ch = (char *)calloc(b + 256,
				  1))) // 256 == 64 * sizeof(int), it's for W.
		return 0;
	rewind(fp);
	...
	do { // 1 -> N blocks of M
		...
		Kp += 48;
		do { // j : 16 -> 63
			*W = Sigma_o1(W[-2]) + W[-7] + Sigma_o0(W[-15]) +
			     W[-16];
			W++;
		} while (Kp != W);
		Kp = K;
		W -= 64;
		do {
			T1 = h + Sigma_E1(e) + Ch(e, f, g) + *W +
			     *Kp; // T1 ← h + Σ1(e) + Ch(e, f, g) + Wj + Kj
			h = g;
			g = f;
			f = e;
			e = d + T1;
	...
	sprintf(buf, "%.8x%.8x%.8x%.8x%.8x%.8x%.8x%.8x", H0, H1, H2, H3, H4, H5,
		H6, H7);
	return buf;
}
